...
export const CountyCode = () => {
  ...
  const totalItems = useAppSelector(state => state.countyCode.totalItems);
  const links = useAppSelector(state => state.countyCode.links);
  const entity = useAppSelector(state => state.countyCode.entity);
      ...
      <div className="table-responsive">
        ...
        >
          {countyCodeList && countyCodeList.length > 0 ? (
            ...
            !loading && <div className="alert alert-warning">No County Codes found</div>
          )}
        </InfiniteScroll>
      </div>
    </div>
