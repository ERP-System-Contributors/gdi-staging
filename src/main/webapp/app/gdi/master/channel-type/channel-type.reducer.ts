...
const apiSearchUrl = 'api/_search/channel-types';

// Actions

export const searchEntities = createAsyncThunk('channelType/search_entity', async ({ query, page, size, sort }: IQueryParams) => {
  const requestUrl = `${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`;
  return axios.get<IChannelType[]>(requestUrl);
});

export const getEntities = createAsyncThunk('channelType/fetch_entity_list', async ({ page, size, sort }: IQueryParams) => {
