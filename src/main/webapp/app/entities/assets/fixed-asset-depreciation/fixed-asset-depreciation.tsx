...
import { searchEntities, getEntities } from './fixed-asset-depreciation.reducer';

export const FixedAssetDepreciation = () => {
  ...
  const fixedAssetDepreciationList = useAppSelector(state => state.fixedAssetDepreciation.entities);
  const loading = useAppSelector(state => state.fixedAssetDepreciation.loading);
  const totalItems = useAppSelector(state => state.fixedAssetDepreciation.totalItems);
  ...
  return (
    <div>
      <h2 id="fixed-asset-depreciation-heading" data-cy="FixedAssetDepreciationHeading">
        ...
        <div className="d-flex justify-content-end">
          ...
          <Link
            to="/fixed-asset-depreciation/new"
            className="btn btn-primary jh-create-entity"
            id="jh-create-entity"
            data-cy="entityCreateButton"
          >
            <FontAwesomeIcon icon="plus" />
            &nbsp; Create a new Fixed Asset Depreciation
          </Link>
        </div>
      </h2>
      <Row>
      ...
      <div className="table-responsive">
        {fixedAssetDepreciationList && fixedAssetDepreciationList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('depreciationDate')}>
                  Depreciation Date <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {fixedAssetDepreciationList.map((fixedAssetDepreciation, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/fixed-asset-depreciation/${fixedAssetDepreciation.id}`} color="link" size="sm">
                  ...
                  <td>
                    {fixedAssetDepreciation.depreciationDate ? (
                      <TextFormat type="date" value={fixedAssetDepreciation.depreciationDate} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      <Button
                        tag={Link}
                        to={`/fixed-asset-depreciation/${fixedAssetDepreciation.id}`}
                        color="info"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/fixed-asset-depreciation/${fixedAssetDepreciation.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/fixed-asset-depreciation/${fixedAssetDepreciation.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
          ...
          !loading && <div className="alert alert-warning">No Fixed Asset Depreciations found</div>
        )}
      </div>
      {totalItems ? (
        <div className={fixedAssetDepreciationList && fixedAssetDepreciationList.length > 0 ? '' : 'd-none'}>
